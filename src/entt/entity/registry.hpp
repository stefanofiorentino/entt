...
class basic_registry {
    ...
    struct ctx_variable<Type>: ctx_variable<> {
        template<typename... Args>
...
public:
    ...
    void prepare(Args &&... args) {
        if(const auto ctype = to_integer(type<Component>()); ctype < pools.size() && pools[ctype].pool) {
            ENTT_ASSERT(!pools[ctype].pool->size());
            pools[ctype].pool.reset();
        }
    ...
    entt::basic_runtime_view<Entity> runtime_view(It first, It last) const {
        ...
        std::transform(first, last, selected.begin(), [this](const component ctype) {
            return (to_integer(ctype) < pools.size() && pools[to_integer(ctype)].pool) ? pools[to_integer(ctype)].pool.get() : nullptr;
        });
    ...
    basic_registry clone(exclude_t<Exclude...> = {}) const {
        ...
        if constexpr(sizeof...(Component) == 0) {
            for(auto pos = pools.size(); pos; --pos) {
                const auto curr = pos - 1;

                if(const auto &pdata = pools[curr]; pdata.assure && ((curr != to_integer(type<Exclude>())) && ...)) {
    ...
    void stomp(const entity_type dst, const basic_registry &other, const entity_type src, exclude_t<Exclude...> = {}) {
        if constexpr(sizeof...(Component) == 0) {
            for(auto pos = pools.size(); pos; --pos) {
                ...
                if(const auto &pdata = pools[curr]; pdata.stomp && ((curr != to_integer(type<Exclude>())) && ...) && pdata.pool->has(src)) {
    ...
    Type & set(Args &&... args) {
        ...
        vars[vtype] = std::make_unique<ctx_variable<Type>>(std::forward<Args>(args)...);
        return static_cast<ctx_variable<Type> &>(*vars[vtype]).value;
    }
    ...
    template<typename Type>
    void unset() {
        if(const auto vtype = context_family::type<std::decay_t<Type>>; vtype < vars.size() && vars[vtype]) {
            vars[vtype].reset();
        }
    }
    ...
    const Type * try_ctx() const {
        const auto vtype = context_family::type<std::decay_t<Type>>;
        return (vtype < vars.size() && vars[vtype]) ? &static_cast<const ctx_variable<Type> &>(*vars[vtype]).value : nullptr;
    }
...
private:
    ...
    std::vector<std::unique_ptr<ctx_variable<>>> vars{};
